...
def parse_cli():
    parser = ArgumentParser(description='Update Script with BTC/ERG price value (nanoErgo & milliBitcoin)')
    parser.add_argument(
        '-s', '--server',
...
def main():
    ...
    price = cmc_api.get_ergo_price('BTC')
    logging.info('BTC/ERG price: %s' % price)

    ergo_update(server_, opts.api_key, int(price*1e6))  # milliBitcoin / nanoErgo
